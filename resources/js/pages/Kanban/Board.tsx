    ...
    const handleDueDateChange = (activityId: number, newDate: string) => {
        router.put(
            ...
            },
            {
                preserveScroll: true,
                onSuccess: () =>
                    showNotification('Fecha de vencimiento actualizada 📅'),
            },
        );
    };

    const canModifyItem = (item: any) => {
        if (canAssign) return true; // Administradores y gerencia pueden modificar todo
        if (item.user_id === auth.user.id) return true; // El creador puede modificar su tarea
        if (item.assigned_user_id === auth.user.id) return true; // El asignado puede moverla de status

        // Si no cumple ninguna, significa que la está viendo solo porque tiene el permiso de "view gerencia"
        return false;
    };
                                                            ...
                                                            {(
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="mt-2 flex items-center gap-1.5">
                                                                        {(() => {
                                                                            // Lógica para saber si la tarea está vencida
                                                                            const isOverdue =
                                                                                item.due_date &&
                                                                                new Date(
                                                                                    item.due_date,
                                                                            ...
                                                                            // Si está vencida y NO está en la columna "done" (finalizada), la pintamos de rojo
                                                                            const dateColor =
                                                                                isOverdue &&
                                                                                columnId !==
                                                                                    'done'
                                                                            ...
                                                                            return (
                                                                                <>
                                                                                    ...
                                                                                    <input
                                                                                        ...
                                                                                        className={`cursor-pointer rounded border-none bg-transparent p-0 px-1 text-xs transition-colors outline-none hover:bg-neutral-100 focus:ring-0 dark:hover:bg-neutral-800 ${dateColor}`}
