...
export default function Board({
  ...
  const handleAssignUser = (activityId: number, newUserId: string) => {
    router.put(
      ...
      {
        preserveScroll: true,
        onSuccess: () => showNotification("Usuario asignado correctamente 👤"),
      },
    );
  };
  ...
  useEffect(() => {
    const initialColumns = {
      ...
      in_progress: {
        name: "En Progreso",
        items: activities.filter((a: any) => a.status === "in_progress"),
      },
      done: {
        name: "Finalizado",
        ...
        <div className="rounded-xl border border-sidebar-border/70 bg-white p-6 shadow-sm dark:border-sidebar-border dark:bg-neutral-900">
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              >
                ...
                <option value="medium" className="dark:bg-neutral-900">
                  Media
                </option>
                <option value="high" className="dark:bg-neutral-900">
                  Alta
                </option>
              </select>
            </div>
          ...
          {categories.length === 0 && (
            <p className="mt-3 text-sm text-red-500">
              ⚠️ Por favor, ve a "Categorías" y crea al menos una antes de
              agregar tareas.
            </p>
          )}
        </div>
                                    ...
                                    return (
                                      <>
                                        ...
                                        <input
                                          ...
                                          // Deshabilitamos si no tiene permiso, igual que el título
                                          disabled={
                                            !canAssign &&
                                            item.user_id !== auth.user.id
                                          }
                                          className={`bg-transparent p-0 text-xs border-none focus:ring-0 cursor-pointer outline-none hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded px-1 transition-colors ${dateColor}`}
